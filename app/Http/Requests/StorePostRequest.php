...
class StorePostRequest extends FormRequest
{
    protected $rules = [
        ...
        'source' => 'exclude_if:is_reproduced,false|required|max:31',
        'source_url' => 'exclude_if:is_reproduced,false|max:255',
    ...
    public function rules()
    {
        ...
        switch ($this->method()) {
            ...
            case 'POST': {
                if ($this->attributes->get('is_reproduced') == '0') {
                    return $rules;
                } else {
                    $rules['source'] = 'required|max:31';
                    return $rules;
                }
            }
            case 'PUT':
            case 'PATCH': {
                $rules['title_cn'] = 'required|max:31|unique:posts,title->cn,' . $this->id;
                $rules['title_en'] = 'required|max:31|unique:posts,title->en,' . $this->id;
                if ($this->attributes->get('is_reproduced') == '0') {
                    return $rules;
                } else {
                    $rules['source'] = 'required|max:31';
    ...
    public function messages()
    {
        return [
            ...
            'details_cn.max' => trans('admin_CRUD.max_limit_250_cn'),
            'details_en.max' => trans('admin_CRUD.max_limit_1500_en'),
